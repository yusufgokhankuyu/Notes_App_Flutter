...
// open the database
  ...
  Future<List<Kategori>> kategoriListesiniGetir() async {
    var kategroiIcerenMapListesi = await kategorileriGetir();
     var kategoriListesi = <Kategori>[];
    for (Map<String, dynamic> map in kategroiIcerenMapListesi) {
  ...
  Future<int> notGuncelle(Not not) async {
    var db = await _getDatabase();
    var sonuc =
        db.update('not', not.toMap(), where: 'notID=?', whereArgs: [not.notID]);
    return sonuc;
  }
